...
{
	CurrentPlayer.x_pos += cos(CurrentPlayer.alpha) / 100.0f * 8;
	CurrentPlayer.y_pos += sin(CurrentPlayer.alpha) / 100.0f * 8;
...
GLboolean Init(void)
{
	...
	texture_grass = auxDIBImageLoad("data\\tGround.bmp");
...
GLfloat ReturnHEIGHT(GLint _x_, GLint _y_)
{
	...
	return sin(_x_) + sin(_x_ / 2) + cos(_y_) + cos(2 * _y_);
}
